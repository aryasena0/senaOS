{ lib, OpenGL, Xplugin, bison, buildPackages, directx-headers, elfutils, expat
, fetchCrate, fetchurl, file, flex, glslang, intltool, jdupes, libdrm, libffi
, libglvnd, libomxil-bellagio, libunwind, libva-minimal, libvdpau, llvmPackages
, lm_sensors, meson, ninja, openssl, pkg-config, python3Packages, rust-bindgen
, rust-cbindgen, rustPlatform, rustc, spirv-llvm-translator, stdenv, udev
, valgrind-light, vulkan-loader, wayland, wayland-protocols, wayland-scanner
, xcbutilkeysyms, xorg, zstd, withValgrind ?
...
, enableGalliumNine ? stdenv.isLinux, enableOSMesa ? stdenv.isLinux
, enableOpenCL ? stdenv.isLinux && stdenv.isx86_64, enableTeflon ?
  stdenv.isLinux
...
, galliumDrivers ? if stdenv.isLinux then
  ...
  [ "auto" ], vulkanDrivers ? if stdenv.isLinux then
    [
      "amd" # AMD (aka RADV)
...
, vulkanLayers ?
  lib.optionals (!stdenv.isDarwin) [ # No Vulkan support on Darwin
    "device-select"
    "overlay"
    "intel-nullhw"
  ] }:
...
let
  ...
  self = stdenv.mkDerivation {
    ...
    patches = [ ./opencl.patch ] ++ lib.optionals stdenv.isDarwin [
    ...
    outputs = [ "out" "dev" "drivers" ]
      ...
      ++ lib.optionals enableOpenCL [ "opencl" ] ++ lib.optionals haveDozen [
